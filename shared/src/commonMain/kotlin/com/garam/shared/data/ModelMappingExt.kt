...
fun NetworkTodo.toLocal(uid: String) = LocalTodo(
    id = id,
    categoryId = categoryId,
    title = title,
    startDate = startDate,
    endDate = endDate,
    repeatRule = repeatRule,
    status = status?.mapValues { entry ->
        val name = entry.value // Map의 값은 Enum의 이름 문자열

        TodoStatus.fromName(name) // 1단계에서 정의한 안전한 함수 호출

    }?.toMutableMap(),
//        status?.mapValues { TodoStatus.valueOf(it.value) }?.toMutableMap(),
    priority = priority,
    memo = memo,
    icon = icon?.IconType(),
    color = color,
...
fun NetworkGoal.toLocal(uid: String) = LocalGoal(
    ...
    startDate = startDate,
    endDate = endDate,
    type = if(type == "WEEKLY") GoalType.WEEKLY else GoalType.MONTHLY,
//        type.let { name ->
////        GoalType.valueOf(it)
//        GoalType.entries.firstOrNull { it.name == name }
//            ?: throw IllegalArgumentException("Invalid GoalType name: $name")
//                    },
    userId = uid
)
